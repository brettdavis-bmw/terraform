...
func assertObjectCompatible(schema *configschema.Block, planned, actual cty.Value, path cty.Path) []error {
	...
	for name, blockS := range schema.BlockTypes {
		...
		maybeUnknownBlocks := couldHaveUnknownBlockPlaceholder(plannedV, blockS)
...
func couldHaveUnknownBlockPlaceholder(v cty.Value, blockS *configschema.NestedBlock) bool {
	switch blockS.Nesting {
	case configschema.NestingSingle, configschema.NestingGroup:
...
func couldBeUnknownBlockPlaceholderElement(v cty.Value, schema *configschema.Block) bool {
	...
	for name, blockS := range schema.BlockTypes {
		if !couldHaveUnknownBlockPlaceholder(v.GetAttr(name), blockS) {
			return false
		}
	}
	return true
